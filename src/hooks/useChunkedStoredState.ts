...
export default function useStoredState<T>(
	...
	const [value, setValue] = useState<T>(() => {
		...
		const initialValue = storedValues.join('').length
			? storedValues.join('')
		...
		try {
			...
			console.warn(
				`Parsing the stored ${key} failed. Purging stored data. Details:`,
				e
			);
			localStorage.clear();
		}
		...
		(newValue, ...args) => {
			...
			try {
				chunks.forEach((chunk, i) =>
					localStorage.setItem(`${key}-chunk-${i}`, chunk)
				);
			} catch (e) {
				alert(
					`The ${key} you selected is too large to get stored permanently. It will get used during this session, but upon refreshing the page, you'll have to reselect it.`
				);
