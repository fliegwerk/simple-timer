...
export default function AppSettings({
	...
	return (
		<div className="container">
			<div>
				<h2>
					<FontAwesomeIcon icon={faParagraph} /> Title
				</h2>
				<div>
					<ContentEditable
						className="textarea"
			...
			<div>
				...
				<div>
					<input
						id="inputImage"
						type="file"
						accept={'.png,.jpg,.jpeg,.gif,.tiff,.svg'}
						onChange={onFileSelected}
					/>
					<label className="textarea" htmlFor="inputImage">
						<FontAwesomeIcon icon={faFolderOpen} /> Select file (
						{image ? image.title : 'none selected'})
					</label>
					{image && <button onClick={clearFile}>Clear</button>}
					<p>
						<small>
							This image only gets used locally. No images ever reach our
							servers.
						</small>
					</p>
				</div>
			...
			<div>
				<h2>
					<FontAwesomeIcon icon={faCalendar} /> Count to datetime
				</h2>
				...
				<button onClick={startCountToDate}>
					Start <FontAwesomeIcon icon={faPlaneDeparture} />
				</button>
			</div>
			<div>
				<h2>
				...
				<button onClick={startCountdown}>
					Start <FontAwesomeIcon icon={faRocket} />
				</button>
			</div>
		</div>
	);
