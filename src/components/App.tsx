...
function App() {
	...
	const [finishDate, setFinishDate] = useStoredDateState(
		'finish-date',
		new Date()
	);
	const [countdownTime, setCountdownTime] = useStoredState<DateMillis>(
		'countdownTime',
		15 * 60 * 1000
	);
	const [infoText, setInfoText] = useStoredState<string>(
		'infoText',
		defaultInfoText
	);
	const [image, setImage] = useChunkedStoredState<FileObj | null>(
		'image',
		16,
		null
	);
		...
		<DarkModeToggle>
			<div className="App">
				<header className="App-header">
					{appState === 'none' && (
						<>
							<h1 className="heading">
								<FontAwesomeIcon icon={faStopwatch} /> Simple Timer
							</h1>
							<AppSettings
					...
					{appState !== 'none' && image && (
						<img id="logo" src={image.dataURI} alt={image.title} />
					)}
